...
const SearchExisting = withState( {
    ...
    const handleChange = (e, d) => {
        ...
        replaceContent(clientId, value, postType, replaceInnerBlocks).then(post => {
            setAttributes({title: text, id: value, lastUpdated: post.modified_gmt});
        });
    }
            ...
            <Form onSubmit={handleCreation}>
              <Form.Group>
                <Form.Input
                  ...
                  onChange={handleChange}
                />
                <Form.Button color="blue" icon="add" content={__('Create ' + postType)} />
              </Form.Group>
            </Form>
        </Fragment>
    );
...
const InitialState = ({postType, setAttributes, className, clientId}) => {
    ...
    return(
        <div className={className}>
            <Card size={'large'}>
                <CardHeader><SearchExisting postType={postType} clientId={clientId} setAttributes={setAttributes}/></CardHeader>
                {/* <CardDivider /> */}
                <CardBody><CreateNew postType={postType} setAttributes={setAttributes}/></CardBody>
            </Card>
        </div>
    );
}
