...
class forms {
	...
	public function scripts() {
        wp_register_script(
            'ajax-login-script',
            BASEPLUGIN_URL . 'assets/js/login-registration.js',
            array('jquery')
        );
        ...
        wp_localize_script( 'ajax-login-script', 'ajax_login_object', array(
            ...
            'loadingmessage' => __('Validating user info, please wait...')
        ));
	}
	...
	public function user_details() {
		...
		?>
		<div id="js-user-details-form">
			<div class="ui header">Profile Information</div>
			<div class="ui equal width form">
				...
				<div class="ui submit button">Update Profile</div>
			</div>
		</div>
		<?php
	}
	...
	public function change_password() {
		...
		?>
		<div id="js-change-password-form">
			...
			<div class="ui equal width form">
				...
				<div class="ui submit button">Change Password</div>
			</div>
		</div>
		<?php
	}
	...
	public function login() {
		...
		?>
		<h3>Login</h3>
		<div class="ui form easylms-login">
			...
			<p><small><a href="<?php echo wp_lostpassword_url();?>">Forgot password?</a></small></p>
			<div><div class="ui primary button">Login</div></div>
		</div>
		<?php
		return ob_get_clean();
	}

	public function registration( $segment = true, $title = false, $description = false ) {
		if ( $this->user_id != false ) {
			return;
		}

		$terms_conditions_link = apply_filters( 'easylms_terms_conditions_link', get_bloginfo('url').'/terms-and-conditions' );
		...
		?>
		<h3>Registration</h3>
		<form class="ui form easylms-registration">
			<?php if ( true === $segment ) {
			...
			<div class="required field">
				<label>Email Address</label>
				<input name="email-address" placeholder="jdoe@usc.edu" type="email">
			</div>
			...
			<div class="field">
				...
				<div class="fields">
					<div class="twelve wide field">
						<input name="address-street" placeholder="Street Address" type="text">
					</div>
					<div class="four wide field">
						<input name="address-suite" placeholder="Apt#" type="text">
					</div>
				</div>
			</div>
			...
			<div class="two fields">
				<div class="field">
					<label>State</label>
					<select class="ui fluid search dropdown js-autoload">
				...
				<div class="field">
					<label>Country</label>
					<div class="ui fluid search selection dropdown js-autoload">
			...
			<div class="required field">
				<div class="ui checkbox">
					...
					<label><a href="<?php echo esc_url( $terms_conditions_link ); ?>">I agree to the Terms and Conditions</a></label>
				</div>
			</div>
			<?php wp_nonce_field( 'ajax-login-nonce', 'security' ); ?>
			<?php if ( true === $segment ) {
			...
			<div><div class="ui primary button">Register</div></div>
			<?php if ( true === $segment ) {
				echo '</div>';
				echo '</div>';
			}?>
...
$forms = new forms();
add_action( 'wp_enqueue_scripts', array($forms, 'scripts') );
